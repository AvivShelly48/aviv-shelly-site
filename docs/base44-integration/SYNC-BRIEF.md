...
1. **`WEBSITE_APP_ID` = `6a2ff9d8f0f6cef4ef4c3d65`** ✅ (אפליקציית האתר קיימת). **`WEBSITE_API_KEY`** — יש להנפיק בהגדרות אפליקציית האתר ב-Base44 ולמסור לצד הניהול בערוץ מאובטח (לא בריפו).
